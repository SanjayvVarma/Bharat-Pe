...
<strong> - Defines strong importance (bold).
...
2. :active
The :active pseudo-class selects and styles an element when it is being activated (clicked or pressed).
...
4. :nth-child()
The :nth-child() pseudo-class allows you to select and style elements based on their position in a parent element.
...
```css
1. ::before and ::after
The ::before and ::after pseudo-elements are used to insert content before and after the content of an element, respectively.
...
The ::first-line pseudo-element selects the first line of a block-level element, and ::first-letter selects the first letter of a block-level element.
...
3. ::selection
The ::selection pseudo-element allows you to style the portion of text that is selected by the user.
